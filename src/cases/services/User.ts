...
export default class UserService {
  ...
  //#region desktop
  async desktopAuth(params: appAuthServiceProps): Promise<defaultResponse> {
    try {
      const salt = await this.userRepository.getSalt({
        login: params.login
      })
      ...
      const verificacao = await this.boletimRepository.verificaTokenDesktop({
        uuid: params.uuid
      })

      if (verificacao && verificacao.count > 0) {
        await this.userRepository.vinculaUsuarioDesktop({
          uuid: params.uuid,
          idusuario: userConfirmed.idusuario
        })
      }
